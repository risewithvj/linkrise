...
class LinkRise_Admin {
	...
	public static function page() {
		...
		// Pagination / search (fixed to 20 per page for consistency/performance)
		$pp       = 20;
		...
		echo '<input type="hidden" name="pp" value="20">';
		...
		if ( $pages > 1 ) {
			...
			for ( $p = 1; $p <= $pages; $p++ ) {
				$url = add_query_arg( array( 'page' => 'linkrise', 'tab' => 'links', 'pg' => $p, 'pp' => 20, 's' => $s ), admin_url( 'admin.php' ) );
